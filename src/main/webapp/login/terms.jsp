<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<link rel="stylesheet" href="banti0.css"> 
...
[class~=register] [class~=btn_confirm] {
    text-align: center;
    padding-top: 10px;
}
...
[class~=register] [class~=btn_confirm] [class~=btn_close] {
    float: none;
    display: inline-block;
    max-width: 150px;
}
...
[class~=register] [class~=btn_confirm] [class~=btn_submit] {
    float: none;
    display: inline-block;
    max-width: 150px;
}
...
<!-- onsubmit="return fregister_submit(this);" -->
    <form  name="fregister" id="fregister" action="<%=request.getContextPath()%>/index.jsp?menugroup=login&menu=hewon_joinn"  method="POST" autocomplete="off">
    ...
    <div class="login-sns sns-wrap-32 sns-wrap-over" id="sns_register">
        <h2>SNS 계정으로 가입(네이버,카카오 아이디로 가입하기)</h2>
        <div class="sns-wrap">
                        <a href="http://bant.co.kr/plugin/social/popup.php?provider=naver&amp;url=%2Fbbs%2Fregister.php" class="sns-icon social_link sns-naver" title="네이버">
                <span class="ico"></span>
                <span class="txt">네이버로 회원가입하기</span>
            </a>
                                    <a href="http://bant.co.kr/plugin/social/popup.php?provider=kakao&amp;url=%2Fbbs%2Fregister.php" class="sns-icon social_link sns-kakao" title="카카오">
                <span class="ico"></span>
...
이 약관은 반티세트(제이글로벌) 회사(전자거래 사업자)가 운영하는 반티세트 사이버 몰(이하 &#034;몰&#034;이라 한다)에서 제공하는 인터넷 관련 서비스(이하 &#034;서비스&#034;라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다. 
※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는한 이 약관을 준용합니다」 


제2조(정의)

① &#034;몰&#034;이란 반티세트 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다. 
...
1. 이 약관은 2014년3월1일 부터 적용됩니다.
    ...
    <div class="btn_confirm">
    	<%-- <a href="<%=request.getContextPath()%>/index.jsp" class="btn_close">취소</a>--%>
    	<button type="button" class="btn_cancel" onclick = "location.href='<%=request.getContextPath()%>/index.jsp;" style="height:30px; background-color:grey;border-radius: 3px; border:none; width: 40px;">취소</button>   	
        <button type="submit" style="height:30px;background-color:white;border-radius: 3px; border-width:1px; ">&nbsp회원가입&nbsp</button>
    </div>
