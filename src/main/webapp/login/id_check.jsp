...
<%
	//비정상적인 요청에 대한 응답 처리
	if(request.getParameter("h_hewonid")==null) {
	...
	//아이 디를 전달받아 hewon 테이블에 저장된 해당 아이디의 회원정보를 검색하여 반환하는
	//DAO 클래스의 메소드 호출
...
<body>
<% if(hewon==null) {//전달받은 아이디가 중복되지 않은 경우 %>
	<div>입력된 <span class="h_hewonid">[<%=h_hewonid %>]</span>는 사용 가능한 아이디입니다.</div>
	<div>
...
<% } else {//전달받은 아이디가 중복된 경우 %>
	...
	<script type="text/javascript">
	function idCheck() {
		var h_hewonid=checkForm.h_hewonid.value;
		if(h_hewonid=="") {
